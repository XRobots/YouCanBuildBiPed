...
int servo3Offset = 30;      // left upper leg - higher number makes the leg longer
int servo4Offset = 0;      // left lower leg - higher number makes the leg longer
...
void loop() {
  ...
  if (currentMillis - previousMillis >= 10) {     // this loop runs every 10ms
      ...
      if (robotMode == 0) {
        ...
        kinematics(1, xMain ,zMain ,0, 0);                   // leg1 - send values to inverse kinematics 
        kinematics(2, xMain ,zMain ,0, 0);                   // leg2 - send values to inverse kinematics 
      }
      else if (robotMode == 2) {
        // pose for walking
        kinematics(1, 0 ,140 ,0, 0);                   // leg1 - send values to inverse kinematics 
        kinematics(2, 0 ,140 ,0, 0);                   // leg2 - send values to inverse kinematics
      }
      else if (robotMode == 3) {
        // dynamic walking      
...
/*
        leg1Time = 120 + (abs(pitch)*2);
        leg1Time = constrain(leg1Time, 125,140);
        leg2Time = leg1Time;
...
*/
        ...
        longLeg = 148 + (abs(pitch));
        longLeg = constrain(longLeg,148,150);

        //shortLeg = 134 + (abs(pitch));
        //shortLeg = constrain(shortLeg,134,136);

        Serial.print(stepTime);
        Serial.print(" , ");
        Serial.print(leg1Time);
        Serial.print(" , ");
        Serial.print(longLeg);
        Serial.print(" , ");
        Serial.println(shortLeg);
        ...
        kinematics(1, xMain ,leg1z ,1, leg1Time);                   // leg1 - send values to inverse kinematics 
        kinematics(2, xMain ,leg2z ,1, leg2Time);                   // leg2 - send values to inverse kinematics
      }      
